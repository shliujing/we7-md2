...
//    }
    ...
    public function doWebTeacher()
    {
        ...
        $imgurl = $this->imgurl();
        if ($_W['ispost']) {
            // todo select 变值赋值给学校 id,name,班级 id,name
            if ($op == 'add') {
                $id = intval($_GPC['id']);
...
//                    'title' => $_GPC['title'],
                ...
                if ($id) {
                    pdo_update($this->modulename . '_school_class', $data, array('id' => $id));
                    $this->message('编辑成功！', $this->createWebUrl('class'));
                } else {
                    pdo_insert($this->modulename . '_school_class', $data);
                    $this->message('添加成功！', $this->createWebUrl('class'));
                }
            } elseif ($op == 'del') {
                $id = $_GPC['id'];
                pdo_delete($this->modulename . '_school_class', array('id' => $id));
...
//                    'title' => $_GPC['title'],
                ...
                if ($id) {
                    ...
                    pdo_insert($this->modulename . '_baby', $data);
                    $this->message('添加成功！', $this->createWebUrl('baby'));
                }
            } elseif ($op == 'del') {
                $id = $_GPC['id'];
                pdo_delete($this->modulename . '_baby', array('id' => $id));
                $data = array(
                    'result' => 1
                );
                return json_encode($data);
                ...
                if (!empty($displayorder)) {
                    foreach ($displayorder as $id => $dis) {
                        if ($dis) {
                            ...
                            pdo_update($this->modulename . '_baby', $update, array(
                                'id' => $id
                            ));
                        }
                    }
                ...
                if (!empty($displayorder)) {
                    foreach ($displayorder as $id => $dis) {
                        if ($dis) {
                            ...
                            pdo_update($this->modulename . '_photos', $update, array(
                                'id' => $id
                            ));
                        }
                    }
