...
<body>
<a href="<?php  echo $_W['siteroot'];?>web/index.php?c=site&a=entry&do=ads&m=hr_album&op=add" class="layui-btn" style="float: right;margin-bottom: 10px;margin-right:10px;">添加广告</a>
  ...
  <tbody>
    <?php  if(is_array($list)) { foreach($list as $row) { ?>
		<tr class="pics">
			...
			<td>
				<span><?php  if($row['ishot'] == 0) { ?>推荐<?php  } else { ?>幻灯<?php  } ?></span>
			<td>
				<span><?php  if($row['type'] == 0) { ?>内部连接<?php  } else if($row['type'] == 1) { ?>网页连接<?php  } else { ?>跳转小程序<?php  } ?></span>
			</td>
			<td>
				<div class="link-group">
					<div class="layui-btn-group">
					  <a  class="layui-btn cubtn" href= "<?php  echo $this->createWebUrl('ads', array('op'=>'add','id' => $row['id']))?>">编辑</a>
...
$(".pics .delpic").on('click',function(){
	...
	layui.use('layer', function(){
		  ...
		  layer.confirm('确定删除此分类么？', {
			  btn: ['确定','取消'] //按钮
			}, function(){
			  //layer.msg('的确很重要', {icon: 1});
				$.post("<?php  echo $_W['siteroot'];?>web/index.php?c=site&a=entry&do=ads&m=hr_album&op=del",{id:pid}, function(data) {
